	...
	public override void OnInspectorGUI ()
	{
			...
			HTGuiTools.BeginGroup ();
			{
				...
				if (t.enabledNGuiMode)
				{
					HTGuiTools.BeginGroup (5);
					{
						...
						if (HTGuiTools.Button ("Add camera", Color.green, 100, false)) t.nGUICameras.Add (null);
		...
		if (t.showSelectInspector)
		{
			HTGuiTools.BeginGroup ();
			{
				...
				if (t.autoSelect)
				{
					...
					HTGuiTools.BeginGroup (5);
					{
						if (HTGuiTools.Button ("Add Camera", Color.green, 100)) t.touchCameras.Add (new ECamera (null, false));
		...
		if (t.showGestureInspector)
		{
			HTGuiTools.BeginGroup ();
			{
				...
				if (t.doubleTapDetection == EasyTouch.DoubleTapDetection.ByTime) t.doubleTapTime = EditorGUILayout.Slider ("Double tap time", t.doubleTapTime, 0.15f, 0.4f);
		...
		if (t.showTwoFingerInspector)
		{
			HTGuiTools.BeginGroup ();
			{
				...
				if (t.enable2FingersGesture)
				{
					...
					if (t.enablePinch) t.minPinchLength = EditorGUILayout.FloatField ("Min pinch length", t.minPinchLength);

					EditorGUILayout.Separator ();

					t.enableTwist = HTGuiTools.Toggle ("Enable twist", t.enableTwist, true);
					if (t.enableTwist) t.minTwistAngle = EditorGUILayout.FloatField ("Min twist angle", t.minTwistAngle);
		...
		if (t.showSecondFingerInspector)
		{
			HTGuiTools.BeginGroup ();
			{
				...
				if (t.enableSimulation)
				{
					...
					if (t.secondFingerTexture == null) t.secondFingerTexture = Resources.Load ("secondFinger") as Texture;
