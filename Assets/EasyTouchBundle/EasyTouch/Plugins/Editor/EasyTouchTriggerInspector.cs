...
public class EasyTouchTriggerInspector : Editor
{
	public override void OnInspectorGUI ()
	{
		...
		for (int i = 1; i < eventNames.Length; i++)
		{
			...
			if (result > -1) TriggerInspector (ev, t);
		}
		#endregion
	...
	void TriggerInspector (EasyTouch.EvtType ev, EasyTouchTrigger t)
	{
		...
		if (folding)
		{
			...
			while (i < t.receivers.Count)
			{
				if (t.receivers[i].eventName == ev)
				{
					...
					if (!t.receivers[i].restricted) t.receivers[i].gameObject = (GameObject)EditorGUILayout.ObjectField ("Other object", t.receivers[i].gameObject, typeof (GameObject), true);
					EditorGUILayout.Space ();
					EditorGUILayout.Space ();

					t.receivers[i].otherReceiver = HTGuiTools.Toggle ("Other receiver", t.receivers[i].otherReceiver, true);
					if (t.receivers[i].otherReceiver) t.receivers[i].gameObjectReceiver = (GameObject)EditorGUILayout.ObjectField ("Receiver", t.receivers[i].gameObjectReceiver, typeof (GameObject), true);
					...
					// Methode helper
					string[] mNames = null;
					if (!t.receivers[i].otherReceiver || (t.receivers[i].otherReceiver && t.receivers[i].gameObjectReceiver == null)) mNames = GetMethod (t.gameObject);
					else if (t.receivers[i].otherReceiver && t.receivers[i].gameObjectReceiver != null) mNames = GetMethod (t.receivers[i].gameObjectReceiver);

					int index = EditorGUILayout.Popup ("", -1, mNames, "Button", GUILayout.Width (20));
					if (index > -1) t.receivers[i].methodName = mNames[index];
					EditorGUILayout.EndHorizontal ();
		...
		else HTGuiTools.BeginGroup ();
		HTGuiTools.EndGroup (false);

		if (!GUILayout.Toggle (true, "+", "ObjectPickerTab")) t.AddTrigger (ev);

		GUILayout.Space (5f);
	}

	void AddEvent (EasyTouch.EvtType ev, EasyTouchTrigger t)
	{
				...
				if (comp != null)
				{
					if (comp.GetType ().IsSubclassOf (typeof (MonoBehaviour)))
					{
						...
						foreach (MethodInfo methodInfo in methodInfos)
						{
							...
							{
								if (methodInfo.IsPublic) methodName.Add (methodInfo.Name);
							}
