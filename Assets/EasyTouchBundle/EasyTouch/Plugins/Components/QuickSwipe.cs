...
namespace HedgehogTeam.EasyTouch
{
	...
	public class QuickSwipe : QuickBase
	{
		...
		public QuickSwipe () => quickActionName = "QuickSwipe" + System.Guid.NewGuid ().ToString ().Substring (0, 7);
			...
			if (gesture.touchCount == 1 && ((gesture.pickedObject != gameObject && !allowSwipeStartOverMe) || allowSwipeStartOverMe))
			{
				...
				if (actionTriggering == ActionTriggering.InProgress)
				{
					if (isRightDirection (gesture))
					{
						...
						if (enableSimpleAction) DoAction ();
					}
				}
			}
		}
		...
		void On_SwipeEnd (Gesture gesture)
		{
			...
			if (fingerIndex == gesture.fingerIndex) fingerIndex = -1;
		}

		void On_DragEnd (Gesture gesture)
		{
			if (gesture.pickedObject == gameObject && allowSwipeStartOverMe) On_SwipeEnd (gesture);
		}

		void On_Drag (Gesture gesture)
		{
			if (gesture.pickedObject == gameObject && allowSwipeStartOverMe) On_Swipe (gesture);
		}
		#endregion
