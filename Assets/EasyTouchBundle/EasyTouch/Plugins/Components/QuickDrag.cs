...
namespace HedgehogTeam.EasyTouch
{
	...
	public class QuickDrag : QuickBase
	{
		...
		{
			if (isStopOncollisionEnter && isOnDrag) StopDrag ();
		}
		#endregion
			...
			if (isOnDrag && fingerIndex == gesture.fingerIndex && realType == GameObjectType.UI)
			{
				if (gesture.isOverGui)
				{
					if ((gesture.pickedUIElement == gameObject || gesture.pickedUIElement.transform.IsChildOf (transform)))
					{
						...
						if (gesture.deltaPosition != Vector2.zero) onDrag.Invoke (gesture);
						lastGesture = gesture;
					}
				}
			}
			...
			if (realType != GameObjectType.UI)
			{
				if ((!enablePickOverUI && gesture.pickedUIElement == null) || enablePickOverUI)
				{
					if (gesture.pickedObject == gameObject && !isOnDrag)
					{
						...
						if (resetPhysic)
						{
							if (cachedRigidBody) cachedRigidBody.isKinematic = true;

							if (cachedRigidBody2D) cachedRigidBody2D.isKinematic = true;
						}
						onDragStart.Invoke (gesture);
					}
				}
		...
		Vector3 GetPositionAxes (Vector3 position)
		{
		...
		public void StopDrag ()
		{
			...
			if (resetPhysic)
			{
				if (cachedRigidBody) cachedRigidBody.isKinematic = isKinematic;

				if (cachedRigidBody2D) cachedRigidBody2D.isKinematic = isKinematic2D;
			}
			isOnDrag = false;
