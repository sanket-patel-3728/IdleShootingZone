...
namespace HedgehogTeam.EasyTouch
{
	...
	public class QuickPinch : QuickBase
	{
		...
		public QuickPinch () => quickActionName = "QuickPinch" + System.Guid.NewGuid ().ToString ().Substring (0, 7);
		...
		public override void OnDisable () => UnsubscribeEvent ();

		void OnDestroy () => UnsubscribeEvent ();
		...
		{
			if (actionTriggering == ActionTiggering.InProgress && pinchDirection == ActionPinchDirection.All) DoAction (gesture);
		}

		void On_PinchIn (Gesture gesture)
		{
			if (actionTriggering == ActionTiggering.InProgress & pinchDirection == ActionPinchDirection.PinchIn) DoAction (gesture);
		}

		void On_PinchOut (Gesture gesture)
		{
			if (actionTriggering == ActionTiggering.InProgress & pinchDirection == ActionPinchDirection.PinchOut) DoAction (gesture);
		}

		void On_PichEnd (Gesture gesture)
		{
			if (actionTriggering == ActionTiggering.End) DoAction (gesture);
		}
				...
				if (realType == GameObjectType.UI)
				{
					if (gesture.isOverGui)
					{
						if ((gesture.pickedUIElement == gameObject || gesture.pickedUIElement.transform.IsChildOf (transform)))
						{
							...
							if (enableSimpleAction) DoDirectAction (axisActionValue);
						}
					}
				}
				else
