...
namespace HedgehogTeam.EasyTouch
{
	...
	public class QuickEnterOverExist : QuickBase
	{
		...
		public QuickEnterOverExist () => quickActionName = "QuickEnterOverExit" + System.Guid.NewGuid ().ToString ().Substring (0, 7);

		void Awake ()
		{
			for (int i = 0; i < 100; i++) fingerOver[i] = false;
		}
			...
			if (realType != GameObjectType.UI)
			{
				if ((!enablePickOverUI && gesture.GetCurrentFirstPickedUIElement () == null) || enablePickOverUI)
				{
					if (gesture.GetCurrentPickedObject () == gameObject)
					{
						...
						else if (fingerOver[gesture.fingerIndex]) onTouchOver.Invoke (gesture);
					}
					else
					{
						if (fingerOver[gesture.fingerIndex])
						{
							...
							if (!isMultiTouch) isOnTouch = false;
						}
					}
				}
				else
