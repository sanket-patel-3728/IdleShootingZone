...
namespace HedgehogTeam.EasyTouch
{
	public class QuickBase : MonoBehaviour
	{
		...
		void Awake ()
		{
			...
			if (cachedRigidBody) isKinematic = cachedRigidBody.isKinematic;

			cachedRigidBody2D = GetComponent<Rigidbody2D> ();
			if (cachedRigidBody2D) isKinematic2D = cachedRigidBody2D.isKinematic;
		}
		...
		public virtual void Start ()
		{
			...
			if (GetComponent<Collider> ()) realType = GameObjectType.Obj_3D;
			else if (GetComponent<Collider2D> ()) realType = GameObjectType.Obj_2D;
			else if (GetComponent<CanvasRenderer> ()) realType = GameObjectType.UI;
		...
		protected void DoDirectAction (float value)
		{
			...
			switch (directAction)
			{
				...
				if (directCharacterController == null) transform.Translate (localAxis * value, Space.World);
				else
				{
				...
				if (directCharacterController == null) transform.Translate (localAxis * value, Space.Self);
				else
				{
