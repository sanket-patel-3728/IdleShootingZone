...
namespace HedgehogTeam.EasyTouch
{
	...
	public class QuickLongTap : QuickBase
	{
		...
		public QuickLongTap () => quickActionName = "QuickLongTap" + System.Guid.NewGuid ().ToString ().Substring (0, 7);
		...
		void Update ()
		{
			...
			if (currentGesture != null)
			{
				...
				{
					if (currentGesture.type == EasyTouch.EvtType.On_LongTapStart2Fingers && actionTriggering == ActionTriggering.Start) DoAction (currentGesture);

					if (currentGesture.type == EasyTouch.EvtType.On_LongTap2Fingers && actionTriggering == ActionTriggering.InProgress) DoAction (currentGesture);

					if (currentGesture.type == EasyTouch.EvtType.On_LongTapEnd2Fingers && actionTriggering == ActionTriggering.End) DoAction (currentGesture);
				}
			}
		}
		...
		{
			if (IsOverMe (gesture)) onLongTap.Invoke (gesture);
		}
