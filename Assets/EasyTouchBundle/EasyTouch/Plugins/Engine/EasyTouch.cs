...
namespace HedgehogTeam.EasyTouch
{
	...
	public class EasyTouch : MonoBehaviour
	{
		...
		Gesture _currentGesture = new Gesture ();
		public static Gesture current => instance._currentGesture;

		List<Gesture> _currentGestures = new List<Gesture> ();
		...
		EasyTouchInput input = new EasyTouchInput ();
		Finger[] fingers = new Finger[100];                 // The informations of the touch for finger 1.
		public Texture secondFingerTexture;                         // The texture to display the simulation of the second finger.
		TwoFingerGesture twoFinger = new TwoFingerGesture ();
		int oldTouchCount = 0;
		DoubleTap[] singleDoubleTap = new DoubleTap[100];
		...
		void OnEnable ()
		{
			if (Application.isPlaying && Application.isEditor) Init ();
		}
		...
		void Start ()
		{
			for (int i = 0; i < 100; i++) singleDoubleTap[i] = new DoubleTap ();
			...
			if (index < 0) touchCameras.Add (new ECamera (Camera.main, false));

			// Fire ready event
			On_EasyTouchIsReady?.Invoke ();
		...
		void OnGUI ()
		{
			if (enableSimulation && !enableRemote)
			{
				...
				if (finger != new Vector2 (-1, -1)) GUI.DrawTexture (new Rect (finger.x - 16, Screen.height - finger.y - 16, 32, 32), secondFingerTexture);
			}
		}
#endif
		...
		void Update ()
		{
			...
			{
				if (Application.isPlaying && Input.touchCount > 0) enableRemote = true;

				if (Application.isPlaying && Input.touchCount == 0) enableRemote = false;
				...
				if (oldTouchCount == 2 && count != 2 && count > 0) CreateGesture2Finger (EvtType.On_Cancel2Fingers, Vector2.zero, Vector2.zero, Vector2.zero, 0, SwipeDirection.None, 0, Vector2.zero, 0, 0, 0);
		...
		{
			// single gesture
			if (_currentGestures.Count > 1) _currentGestures.RemoveAt (0);
			else _currentGestures[0] = null;
			...
			if (realTouch || enableRemote)
			{
				...
				for (var i = 0; i < touchCount; ++i)
				{
					...
					while (t < 100 && fingers[i] == null)
					{
						if (tmpArray[t] != null)
						{
							if (tmpArray[t].fingerIndex == touch.fingerId) fingers[i] = tmpArray[t];
						}
						t++;
					}
					...
					else fingers[i].phase = touch.phase;

					if (fingers[i].phase != TouchPhase.Began) fingers[i].deltaPosition = touch.position - fingers[i].position;
					else fingers[i].deltaPosition = Vector2.zero;
		...
		void ResetTouches ()
		{
			for (int i = 0; i < 100; i++) fingers[i] = null;
		}
		#endregion

		#region One finger Private methods
		void OneFinger (int fingerIndex)
		{
			// A tap starts ?
			...
			if (fingers[fingerIndex].phase != TouchPhase.Ended && fingers[fingerIndex].phase != TouchPhase.Canceled)
			{
				...
				if (((fingers[fingerIndex].gesture == GestureType.Acquisition || fingers[fingerIndex].gesture == GestureType.LongTap) && fingers[fingerIndex].phase == TouchPhase.Moved && gesturePriority == GesturePriority.Slips) || ((fingers[fingerIndex].gesture == GestureType.Acquisition || fingers[fingerIndex].gesture == GestureType.LongTap) && (FingerInTolerance (fingers[fingerIndex]) == false) && gesturePriority == GesturePriority.Tap))
				{
					...
					else
					{
						...
						if (fingers[fingerIndex].pickedObject)
						{
							...
							if (alwaysSendSwipe) CreateGesture (fingerIndex, EvtType.On_SwipeStart, fingers[fingerIndex], SwipeDirection.None, 0, Vector2.zero);
						}
						// If not swipe
						else
						{
				...
				if (message != EvtType.None)
				{
					...
					if (message == EvtType.On_Drag && alwaysSendSwipe) CreateGesture (fingerIndex, EvtType.On_Swipe, fingers[fingerIndex], currentSwipe, 0, fingers[fingerIndex].deltaPosition);
				}
			...
			else
			{
				...
				switch (fingers[fingerIndex].gesture)
				{
					...
					if (doubleTapDetection == DoubleTapDetection.BySystem)
					{
						...
						{
							if (fingers[fingerIndex].tapCount < 2) CreateGesture (fingerIndex, EvtType.On_SimpleTap, fingers[fingerIndex], SwipeDirection.None, 0, Vector2.zero);
							else CreateGesture (fingerIndex, EvtType.On_DoubleTap, fingers[fingerIndex], SwipeDirection.None, 0, Vector2.zero);
					...
					else
					{
						...
						else singleDoubleTap[fingerIndex].count++;
					}
					...
					if (alwaysSendSwipe) CreateGesture (fingerIndex, EvtType.On_SwipeEnd, fingers[fingerIndex], GetSwipe (fingers[fingerIndex].startPosition, fingers[fingerIndex].position), (fingers[fingerIndex].position - fingers[fingerIndex].startPosition).magnitude, fingers[fingerIndex].position - fingers[fingerIndex].startPosition);
					break;
					// swipe
					case GestureType.Swipe:
		...
		IEnumerator SingleOrDouble (int fingerIndex)
		{
			...
			if (singleDoubleTap[fingerIndex].count < 2) CreateGesture (fingerIndex, EvtType.On_SimpleTap, singleDoubleTap[fingerIndex].finger, SwipeDirection.None, 0, singleDoubleTap[fingerIndex].finger.deltaPosition);
			else CreateGesture (fingerIndex, EvtType.On_DoubleTap, singleDoubleTap[fingerIndex].finger, SwipeDirection.None, 0, singleDoubleTap[fingerIndex].finger.deltaPosition);
		...
		void CreateGesture (int touchIndex, EvtType message, Finger finger, SwipeDirection swipe, float swipeLength, Vector2 swipeVector)
		{
			...
			// NGui
			if (enabledNGuiMode && message == EvtType.On_TouchStart) finger.isOverGui = finger.isOverGui || IsTouchOverNGui (finger.position);

			// firing event ?
			if ((enableUIMode || enabledNGuiMode)) firingEvent = !finger.isOverGui;
			...
			{
				if (message == EvtType.On_TouchUp) RaiseEvent (EvtType.On_UIElementTouchUp, gesture);
				else RaiseEvent (EvtType.On_OverUIElement, gesture);
			}
		...
		void TwoFinger ()
		{
			...
			if (twoFinger.currentGesture == GestureType.None)
			{
				...
				if (autoSelect)
				{
					...
					else twoFinger.ClearPickedObjectData ();
				}
				...
				if (allowUIDetection)
				{
					...
					else twoFinger.ClearPickedUIData ();
				}
			...
			// Cancel
			if (fingers[twoFinger.finger0].phase == TouchPhase.Canceled || fingers[twoFinger.finger1].phase == TouchPhase.Canceled) twoFinger.currentGesture = GestureType.Cancel;
			...
			if (fingers[twoFinger.finger0].phase != TouchPhase.Ended && fingers[twoFinger.finger1].phase != TouchPhase.Ended && twoFinger.currentGesture != GestureType.Cancel)
			{
				...
				// Let's move us ?
				if (((FingerInTolerance (fingers[twoFinger.finger0]) == false || FingerInTolerance (fingers[twoFinger.finger1]) == false) && gesturePriority == GesturePriority.Tap) || ((fingers[twoFinger.finger0].phase == TouchPhase.Moved || fingers[twoFinger.finger1].phase == TouchPhase.Moved) && gesturePriority == GesturePriority.Slips)) move = true;
				...
				if (move && twoFinger.currentGesture != GestureType.Tap)
				{
					...
					if (enable2FingersSwipe)
					{
						...
						if (twoFinger.swipeStart) CreateGesture2Finger (EvtType.On_Swipe2Fingers, twoFinger.startPosition, twoFinger.position, twoFinger.deltaPosition, twoFinger.timeSinceStartAction, GetSwipe (twoFinger.oldStartPosition, twoFinger.position), 0, twoFinger.deltaPosition, 0, 0, twoFinger.fingerDistance);
				...
				{
					// Long tap update
					if (twoFinger.currentGesture == GestureType.LongTap) CreateGesture2Finger (EvtType.On_LongTap2Fingers, twoFinger.startPosition, twoFinger.position, twoFinger.deltaPosition, twoFinger.timeSinceStartAction, SwipeDirection.None, 0, Vector2.zero, 0, 0, twoFinger.fingerDistance);
				}
		...
		void DetectPinch (float currentDelta)
		{
			#region Pinch
			if (enablePinch)
			{
				if ((Mathf.Abs (twoFinger.fingerDistance - twoFinger.startDistance) >= minPinchLength && twoFinger.currentGesture != GestureType.Pinch) || twoFinger.currentGesture == GestureType.Pinch)
				{
					...
					if (currentDelta > 0) CreateGesture2Finger (EvtType.On_PinchOut, twoFinger.startPosition, twoFinger.position, twoFinger.deltaPosition, twoFinger.timeSinceStartAction, GetSwipe (twoFinger.startPosition, twoFinger.position), 0, Vector2.zero, 0, Mathf.Abs (twoFinger.fingerDistance - twoFinger.oldFingerDistance), twoFinger.fingerDistance);

					if (currentDelta < 0) CreateGesture2Finger (EvtType.On_PinchIn, twoFinger.startPosition, twoFinger.position, twoFinger.deltaPosition, twoFinger.timeSinceStartAction, GetSwipe (twoFinger.startPosition, twoFinger.position), 0, Vector2.zero, 0, Mathf.Abs (twoFinger.fingerDistance - twoFinger.oldFingerDistance), twoFinger.fingerDistance);

					if (currentDelta < 0 || currentDelta > 0) CreateGesture2Finger (EvtType.On_Pinch, twoFinger.startPosition, twoFinger.position, twoFinger.deltaPosition, twoFinger.timeSinceStartAction, GetSwipe (twoFinger.startPosition, twoFinger.position), 0, Vector2.zero, 0, currentDelta, twoFinger.fingerDistance);
				}
		...
		void DetecTwist (Vector2 previousDistance, Vector2 currentDistance, float currentDelta)
		{
			#region Twist
			if (enableTwist)
			{
				...
				if (previousDistance == currentDistance) twistAngle = 0;
				...
				if (Mathf.Abs (twistAngle) >= minTwistAngle && (twoFinger.currentGesture != GestureType.Twist) || twoFinger.currentGesture == GestureType.Twist)
				{
					...
					if (twistAngle != 0) twistAngle *= Mathf.Sign (Vector3.Cross (previousDistance, currentDistance).z);
		...
		void CreateStateEnd2Fingers (GestureType gesture, Vector2 startPosition, Vector2 position, Vector2 deltaPosition, float time, bool realEnd, float fingerDistance, float twist = 0, float pinch = 0)
		{
			switch (gesture)
			{
				// Tap
				...
				if (doubleTapDetection == DoubleTapDetection.BySystem)
				{
					if (fingers[twoFinger.finger0].tapCount < 2 && fingers[twoFinger.finger1].tapCount < 2) CreateGesture2Finger (EvtType.On_SimpleTap2Fingers, startPosition, position, deltaPosition, time, SwipeDirection.None, 0, Vector2.zero, 0, 0, fingerDistance);
					else CreateGesture2Finger (EvtType.On_DoubleTap2Fingers, startPosition, position, deltaPosition, time, SwipeDirection.None, 0, Vector2.zero, 0, 0, fingerDistance);
				...
				{
					if (!singleDoubleTap[99].inWait) StartCoroutine ("SingleOrDouble2Fingers");
					else singleDoubleTap[99].count++;
				}
				break;

				// Long tap
				case GestureType.LongTap:
				CreateGesture2Finger (EvtType.On_LongTapEnd2Fingers, startPosition, position, deltaPosition, time, SwipeDirection.None, 0, Vector2.zero, 0, 0, fingerDistance);
				break;

				// Pinch 
				case GestureType.Pinch:
				CreateGesture2Finger (EvtType.On_PinchEnd, startPosition, position, deltaPosition, time, SwipeDirection.None, 0, Vector2.zero, 0, twoFinger.lastPinch, fingerDistance);
				break;

				// twist
				case GestureType.Twist:
				CreateGesture2Finger (EvtType.On_TwistEnd, startPosition, position, deltaPosition, time, SwipeDirection.None, 0, Vector2.zero, twoFinger.lastTwistAngle, 0, fingerDistance);
				break;
			}
			...
			{
				// Drag
				if (twoFinger.dragStart) CreateGesture2Finger (EvtType.On_DragEnd2Fingers, startPosition, position, deltaPosition, time, GetSwipe (startPosition, position), (position - startPosition).magnitude, position - startPosition, 0, 0, fingerDistance);

				// Swipe
				if (twoFinger.swipeStart) CreateGesture2Finger (EvtType.On_SwipeEnd2Fingers, startPosition, position, deltaPosition, time, GetSwipe (startPosition, position), (position - startPosition).magnitude, position - startPosition, 0, 0, fingerDistance);
		...
		IEnumerator SingleOrDouble2Fingers ()
		{
			...
			if (singleDoubleTap[99].count < 2) CreateGesture2Finger (EvtType.On_SimpleTap2Fingers, twoFinger.startPosition, twoFinger.position, twoFinger.deltaPosition, twoFinger.timeSinceStartAction, SwipeDirection.None, 0, Vector2.zero, 0, 0, twoFinger.fingerDistance);
			else CreateGesture2Finger (EvtType.On_DoubleTap2Fingers, twoFinger.startPosition, twoFinger.position, twoFinger.deltaPosition, twoFinger.timeSinceStartAction, SwipeDirection.None, 0, Vector2.zero, 0, 0, twoFinger.fingerDistance);
		...
		void CreateGesture2Finger (EvtType message, Vector2 startPosition, Vector2 position, Vector2 deltaPosition, float actionTime, SwipeDirection swipe, float swipeLength, Vector2 swipeVector, float twist, float pinch, float twoDistance)
		{
			bool firingEvent = true;
			...
			// NGui
			if (enabledNGuiMode && message == EvtType.On_TouchStart2Fingers) gesture.isOverGui = gesture.isOverGui || (IsTouchOverNGui (twoFinger.position) && IsTouchOverNGui (twoFinger.position));
			...
			{
				if (message != EvtType.On_Drag && message != EvtType.On_DragEnd && message != EvtType.On_Twist && message != EvtType.On_TwistEnd && message != EvtType.On_Pinch && message != EvtType.On_PinchEnd && message != EvtType.On_PinchIn && message != EvtType.On_PinchOut)
				{
					if (GetTwoFingerPickedObject ())
					{
						gesture.pickedObject = pickedObject.pickedObj;
			...
			// Firing event ?
			if ((enableUIMode || (enabledNGuiMode && allowUIDetection))) firingEvent = !gesture.isOverGui;

			// Firing event
			if (firingEvent) RaiseEvent (message, gesture);
			else if (gesture.isOverGui)
			{
				if (message == EvtType.On_TouchUp2Fingers) RaiseEvent (EvtType.On_UIElementTouchUp, gesture);
				else RaiseEvent (EvtType.On_OverUIElement, gesture);
			}
		}

		int GetTwoFinger (int index)
		{
			int i = index + 1;
			bool find = false;
		...
		bool GetTwoFingerPickedObject ()
		{
			...
			if (twoFingerPickMethod == TwoFingerPickMethod.Finger)
			{
				if (GetPickedGameObject (fingers[twoFinger.finger0], false))
				{
					...
					{
						if (tmp == pickedObject.pickedObj) returnValue = true;
					}
				}
			}
			...
			{
				if (GetPickedGameObject (fingers[twoFinger.finger0], true)) returnValue = true;
			}
			return returnValue;
		}

		bool GetTwoFingerPickedUIElement ()
		{
			bool returnValue = false;

			if (fingers[twoFinger.finger0] == null) return false;
		...
		void RaiseEvent (EvtType evnt, Gesture gesture)
		{
			gesture.type = evnt;
			...
			switch (evnt)
			{
				...
				On_Cancel?.Invoke (gesture);
				break;
				case EvtType.On_Cancel2Fingers:
				On_Cancel2Fingers?.Invoke (gesture);
				break;
				case EvtType.On_TouchStart:
				On_TouchStart?.Invoke (gesture);
				break;
				case EvtType.On_TouchDown:
				On_TouchDown?.Invoke (gesture);
				break;
				case EvtType.On_TouchUp:
				On_TouchUp?.Invoke (gesture);
				break;
				case EvtType.On_SimpleTap:
				On_SimpleTap?.Invoke (gesture);
				break;
				case EvtType.On_DoubleTap:
				On_DoubleTap?.Invoke (gesture);
				break;
				case EvtType.On_LongTapStart:
				On_LongTapStart?.Invoke (gesture);
				break;
				case EvtType.On_LongTap:
				On_LongTap?.Invoke (gesture);
				break;
				case EvtType.On_LongTapEnd:
				On_LongTapEnd?.Invoke (gesture);
				break;
				case EvtType.On_DragStart:
				On_DragStart?.Invoke (gesture);
				break;
				case EvtType.On_Drag:
				On_Drag?.Invoke (gesture);
				break;
				case EvtType.On_DragEnd:
				On_DragEnd?.Invoke (gesture);
				break;
				case EvtType.On_SwipeStart:
				On_SwipeStart?.Invoke (gesture);
				break;
				case EvtType.On_Swipe:
				On_Swipe?.Invoke (gesture);
				break;
				case EvtType.On_SwipeEnd:
				On_SwipeEnd?.Invoke (gesture);
				break;
				case EvtType.On_TouchStart2Fingers:
				On_TouchStart2Fingers?.Invoke (gesture);
				break;
				case EvtType.On_TouchDown2Fingers:
				On_TouchDown2Fingers?.Invoke (gesture);
				break;
				case EvtType.On_TouchUp2Fingers:
				On_TouchUp2Fingers?.Invoke (gesture);
				break;
				case EvtType.On_SimpleTap2Fingers:
				On_SimpleTap2Fingers?.Invoke (gesture);
				break;
				case EvtType.On_DoubleTap2Fingers:
				On_DoubleTap2Fingers?.Invoke (gesture);
				break;
				case EvtType.On_LongTapStart2Fingers:
				On_LongTapStart2Fingers?.Invoke (gesture);
				break;
				case EvtType.On_LongTap2Fingers:
				On_LongTap2Fingers?.Invoke (gesture);
				break;
				case EvtType.On_LongTapEnd2Fingers:
				On_LongTapEnd2Fingers?.Invoke (gesture);
				break;
				case EvtType.On_Twist:
				On_Twist?.Invoke (gesture);
				break;
				case EvtType.On_TwistEnd:
				On_TwistEnd?.Invoke (gesture);
				break;
				case EvtType.On_Pinch:
				On_Pinch?.Invoke (gesture);
				break;
				case EvtType.On_PinchIn:
				On_PinchIn?.Invoke (gesture);
				break;
				case EvtType.On_PinchOut:
				On_PinchOut?.Invoke (gesture);
				break;
				case EvtType.On_PinchEnd:
				On_PinchEnd?.Invoke (gesture);
				break;
				case EvtType.On_DragStart2Fingers:
				On_DragStart2Fingers?.Invoke (gesture);
				break;
				case EvtType.On_Drag2Fingers:
				On_Drag2Fingers?.Invoke (gesture);
				break;
				case EvtType.On_DragEnd2Fingers:
				On_DragEnd2Fingers?.Invoke (gesture);
				break;
				case EvtType.On_SwipeStart2Fingers:
				On_SwipeStart2Fingers?.Invoke (gesture);
				break;
				case EvtType.On_Swipe2Fingers:
				On_Swipe2Fingers?.Invoke (gesture);
				break;
				case EvtType.On_SwipeEnd2Fingers:
				On_SwipeEnd2Fingers?.Invoke (gesture);
				break;
				case EvtType.On_OverUIElement:
				On_OverUIElement?.Invoke (gesture);
				break;
				case EvtType.On_UIElementTouchUp:
				On_UIElementTouchUp?.Invoke (gesture);
				break;
			}
			...
			if (result == -1)
			{
				...
				if (_currentGestures.Count > 0) _currentGesture = _currentGestures[0];
			}
		}

		bool GetPickedGameObject (Finger finger, bool isTowFinger = false)
		{
			if (finger == null && !isTowFinger) return false;
			...
			if (touchCameras.Count > 0)
			{
				for (int i = 0; i < touchCameras.Count; i++)
				{
					if (touchCameras[i].camera != null && touchCameras[i].camera.enabled)
					{
						...
						if (!isTowFinger) pos = finger.position;
						else pos = twoFinger.position;

						if (GetGameObjectAt (pos, touchCameras[i].camera, touchCameras[i].guiCamera)) return true;
					}
				}
			}
			else Debug.LogWarning ("No camera is assigned to EasyTouch");
			return false;
		}

		bool GetGameObjectAt (Vector2 position, Camera cam, bool isGuiCam)
		{
		...
		SwipeDirection GetSwipe (Vector2 start, Vector2 end)
		{

			Vector2 linear;
			linear = (end - start).normalized;

			if (Vector2.Dot (linear, Vector2.up) >= swipeTolerance) return SwipeDirection.Up;

			if (Vector2.Dot (linear, -Vector2.up) >= swipeTolerance) return SwipeDirection.Down;

			if (Vector2.Dot (linear, Vector2.right) >= swipeTolerance) return SwipeDirection.Right;

			if (Vector2.Dot (linear, -Vector2.right) >= swipeTolerance) return SwipeDirection.Left;

			if (Vector2.Dot (linear, new Vector2 (0.5f, 0.5f).normalized) >= swipeTolerance) return SwipeDirection.UpRight;

			if (Vector2.Dot (linear, new Vector2 (0.5f, -0.5f).normalized) >= swipeTolerance) return SwipeDirection.DownRight;

			if (Vector2.Dot (linear, new Vector2 (-0.5f, 0.5f).normalized) >= swipeTolerance) return SwipeDirection.UpLeft;

			if (Vector2.Dot (linear, new Vector2 (-0.5f, -0.5f).normalized) >= swipeTolerance) return SwipeDirection.DownLeft;

			return SwipeDirection.Other;
		}

		bool FingerInTolerance (Finger finger)
		{
			if ((finger.position - finger.startPosition).sqrMagnitude <= (StationaryTolerance * StationaryTolerance)) return true;
			else return false;
		}

		bool IsTouchOverNGui (Vector2 position, bool isTwoFingers = false)
		{
			bool returnValue = false;
		...
		Finger GetFinger (int finderId)
		{
			...
			while (t < 10 && fing == null)
			{
				if (fingers[t] != null)
				{
					if (fingers[t].fingerIndex == finderId) fing = fingers[t];
				}
				t++;
			}
			return fing;
		...
		#region Unity UI
		bool IsScreenPositionOverUI (Vector2 position)
		{
			...
			if (uiEventSystem != null)
			{
				...
				if (uiRaycastResultCache.Count > 0) return true;
				else return false;
			}
			else return false;
		}
		...
		GameObject GetFirstUIElement (Vector2 position)
		{
			if (IsScreenPositionOverUI (position)) return GetFirstUIElementFromCache ();
			else return null;
		}
		#endregion
		...
		public static bool IsFingerOverUIElement (int fingerIndex)
		{
			if (instance != null)
			{
				...
				if (finger != null) return instance.IsScreenPositionOverUI (finger.position);
				else return false;
			}
			else return false;
		}
		...
		public static GameObject GetCurrentPickedUIElement (int fingerIndex, bool isTwoFinger)
		{
			if (instance != null)
			{
				...
				{
					Vector2 pos = Vector2.zero;
					if (!isTwoFinger) pos = finger.position;
					else pos = instance.twoFinger.position;
					return instance.GetFirstUIElement (pos);
				}
				else return null;
			}
			else return null;
		...
		public static GameObject GetCurrentPickedObject (int fingerIndex, bool isTwoFinger)
		{
			if (instance != null)
			{
				...
				if ((finger != null || isTwoFinger) && instance.GetPickedGameObject (finger, isTwoFinger)) return instance.pickedObject.pickedObj;
				else return null;
			}
			else return null;
		}
		...
		public static GameObject GetGameObjectAt (Vector2 position, bool isTwoFinger = false)
		{
			if (instance != null)
			{
				if (isTwoFinger) position = instance.twoFinger.position;
				if (instance.touchCameras.Count > 0)
				{
					for (int i = 0; i < instance.touchCameras.Count; i++)
					{
						...
						{
							if (instance.GetGameObjectAt (position, instance.touchCameras[i].camera, instance.touchCameras[i].guiCamera)) return instance.pickedObject.pickedObj;
							else return null;
						}
					}
				}
		...
		{
			if (instance) return instance.input.TouchCount ();
			else return 0;
		}

		public static void ResetTouch (int fingerIndex)
		{
			if (instance) instance.GetFinger (fingerIndex).gesture = GestureType.None;
		}
		...
		{
			if (instance) return instance.enable;
			else return false;
		}

		public static void SetEnableUIDetection (bool enable)
		{
			if (instance != null) instance.allowUIDetection = enable;
		}

		public static bool GetEnableUIDetection ()
		{
			if (instance) return instance.allowUIDetection;
			else return false;
		}

		public static void SetUICompatibily (bool value)
		{
			if (instance != null) instance.enableUIMode = value;
		}

		public static bool GetUIComptability ()
		{
			if (instance != null) return instance.enableUIMode;
			else return false;
		}

		public static void SetAutoUpdateUI (bool value)
		{
			if (instance) instance.autoUpdatePickedUI = value;
		}
		public static bool GetAutoUpdateUI ()
		{
			if (instance) return instance.autoUpdatePickedUI;
			else return false;
		}

		public static void SetNGUICompatibility (bool value)
		{
			if (instance) instance.enabledNGuiMode = value;
		}
		public static bool GetNGUICompatibility ()
		{
			if (instance) return instance.enabledNGuiMode;
			else return false;
		}

		public static void SetEnableAutoSelect (bool value)
		{
			if (instance) instance.autoSelect = value;
		}
		public static bool GetEnableAutoSelect ()
		{
			if (instance) return instance.autoSelect;
			else return false;
		}

		public static void SetAutoUpdatePickedObject (bool value)
		{
			if (instance) instance.autoUpdatePickedObject = value;
		}
		public static bool GetAutoUpdatePickedObject ()
		{
			if (instance) return instance.autoUpdatePickedObject;
			else return false;
		}

		public static void Set3DPickableLayer (LayerMask mask)
		{
			if (instance) instance.pickableLayers3D = mask;
		}
		public static LayerMask Get3DPickableLayer ()
		{
			if (instance) return instance.pickableLayers3D;
			else return LayerMask.GetMask ("Default");
		}

		public static void AddCamera (Camera cam, bool guiCam = false)
		{
			if (instance) instance.touchCameras.Add (new ECamera (cam, guiCam));
		}
		public static void RemoveCamera (Camera cam)
		{
			if (instance)
			{
				int result = instance.touchCameras.FindIndex (
					delegate (ECamera c)
					{
						return c.camera == cam;
					}
				...
				if (result > -1)
				{
					instance.touchCameras[result] = null;
					instance.touchCameras.RemoveAt (result);
				}
			}
		}
		...
		{
			if (instance) instance.enable2D = value;
		}
		public static bool GetEnable2DCollider ()
		{
			if (instance) return instance.enable2D;
			else return false;
		}

		public static void Set2DPickableLayer (LayerMask mask)
		{
			if (instance) instance.pickableLayers2D = mask;
		}
		public static LayerMask Get2DPickableLayer ()
		{
			if (instance) return instance.pickableLayers2D;
			else return LayerMask.GetMask ("Default");
		}

		public static void SetGesturePriority (GesturePriority value)
		{
			if (instance) instance.gesturePriority = value;
		}
		public static GesturePriority GetGesturePriority ()
		{
			if (instance) return instance.gesturePriority;
			else return GesturePriority.Tap;
		}

		public static void SetStationaryTolerance (float tolerance)
		{
			if (instance) instance.StationaryTolerance = tolerance;
		}

		public static float GetStationaryTolerance ()
		{
			if (instance) return instance.StationaryTolerance;
			else return -1;
		}

		public static void SetLongTapTime (float time)
		{
			if (instance) instance.longTapTime = time;
		}
		public static float GetlongTapTime ()
		{
			if (instance) return instance.longTapTime;
			else return -1;
		}

		public static void SetDoubleTapTime (float time)
		{
			if (instance) instance.doubleTapTime = time;
		}

		public static float GetDoubleTapTime ()
		{
			if (instance) return instance.doubleTapTime;
			else return -1;
		}

		public static void SetDoubleTapMethod (DoubleTapDetection detection)
		{
			if (instance) instance.doubleTapDetection = detection;
		}

		public static DoubleTapDetection GetDoubleTapMethod ()
		{
			if (instance) return instance.doubleTapDetection;
			else return DoubleTapDetection.BySystem;
		}

		public static void SetSwipeTolerance (float tolerance)
		{
			if (instance) instance.swipeTolerance = tolerance;
		}
		public static float GetSwipeTolerance ()
		{
			if (instance) return instance.swipeTolerance;
			else return -1;
		}

		public static void SetEnable2FingersGesture (bool enable)
		{
			if (instance) instance.enable2FingersGesture = enable;
		}
		public static bool GetEnable2FingersGesture ()
		{
			if (instance) return instance.enable2FingersGesture;
			else return false;
		}

		public static void SetTwoFingerPickMethod (TwoFingerPickMethod pickMethod)
		{
			if (instance) instance.twoFingerPickMethod = pickMethod;
		}
		public static TwoFingerPickMethod GetTwoFingerPickMethod ()
		{
			if (instance) return instance.twoFingerPickMethod;
			else return TwoFingerPickMethod.Finger;
		}

		public static void SetEnablePinch (bool enable)
		{
			if (instance) instance.enablePinch = enable;
		}

		public static bool GetEnablePinch ()
		{
			if (instance) return instance.enablePinch;
			else return false;
		}

		public static void SetMinPinchLength (float length)
		{
			if (instance) instance.minPinchLength = length;
		}
		public static float GetMinPinchLength ()
		{
			if (instance) return instance.minPinchLength;
			else return -1;
		}

		public static void SetEnableTwist (bool enable)
		{
			if (instance) instance.enableTwist = enable;
		}

		public static bool GetEnableTwist ()
		{
			if (instance) return instance.enableTwist;
			else return false;
		}

		public static void SetMinTwistAngle (float angle)
		{
			if (instance) instance.minTwistAngle = angle;
		}

		public static float GetMinTwistAngle ()
		{
			if (instance) return instance.minTwistAngle;
			else return -1;
		}

		public static bool GetSecondeFingerSimulation ()
		{
			if (instance != null) return instance.enableSimulation;
			else return false;
		}

		public static void SetSecondFingerSimulation (bool value)
		{
			if (instance != null) instance.enableSimulation = value;
		}
		#endregion
	}
}