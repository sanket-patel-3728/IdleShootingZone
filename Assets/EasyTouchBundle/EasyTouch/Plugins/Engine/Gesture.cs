...
namespace HedgehogTeam.EasyTouch
{
	...
	public class Gesture : BaseFinger, ICloneable
	{
		...
		public Vector3 GetTouchToWorldPoint (Vector3 position3D) => Camera.main.ScreenToWorldPoint (new Vector3 (position.x, position.y, Camera.main.transform.InverseTransformPoint (position3D).z));
		...
		/// </returns>
		public float GetSwipeOrDragAngle () => Mathf.Atan2 (swipeVector.normalized.y, swipeVector.normalized.x) * Mathf.Rad2Deg;
		...
		/// The position.
		/// </returns>
		public Vector2 NormalizedPosition () => new Vector2 (100f / Screen.width * position.x / 100f, 100f / Screen.height * position.y / 100f);
		...
		public bool IsOverUIElement () => EasyTouch.IsFingerOverUIElement (fingerIndex);
		...
		{
			if (camera == null) return RectTransformUtility.RectangleContainsScreenPoint (tr, position, null);
			else return RectTransformUtility.RectangleContainsScreenPoint (tr, position, camera);
		}
		...
		public GameObject GetCurrentFirstPickedUIElement (bool isTwoFinger = false) => EasyTouch.GetCurrentPickedUIElement (fingerIndex, isTwoFinger);

		/// <summary>
		/// Gets the current picked object.
		/// </summary>
		/// <returns>The current picked object.</returns>
		public GameObject GetCurrentPickedObject (bool isTwoFinger = false) => EasyTouch.GetCurrentPickedObject (fingerIndex, isTwoFinger);
		#endregion
	}
}