...
namespace HedgehogTeam.EasyTouch
{
	...
	public class EasyTouchInput
	{
		...
		Vector2[] oldMousePosition = new Vector2[2];
		int[] tapCount = new int[2];
		float[] startActionTime = new float[2];
		float[] deltaTime = new float[2];
		float[] tapeTime = new float[2];
		...
		int getTouchCount (bool realTouch)
		{
			int count = 0;

			if (realTouch || EasyTouch.instance.enableRemote) count = Input.touchCount;
			else
			{
				if (Input.GetMouseButton (0) || Input.GetMouseButtonUp (0))
				{
					...
					{
						if (Input.GetKey (KeyCode.LeftAlt) || Input.GetKey (EasyTouch.instance.twistKey) || Input.GetKey (KeyCode.LeftControl) || Input.GetKey (EasyTouch.instance.swipeKey)) count = 2;
						if (Input.GetKeyUp (KeyCode.LeftAlt) || Input.GetKeyUp (EasyTouch.instance.twistKey) || Input.GetKeyUp (KeyCode.LeftControl) || Input.GetKeyUp (EasyTouch.instance.swipeKey)) count = 2;
					}
					if (count == 0)
					{
		...
		public Finger GetMouseTouch (int fingerIndex, Finger myFinger)
		{
			...
			if (myFinger != null) finger = myFinger;
			else
			{
				finger = new Finger ();
			...
			if (Input.GetMouseButton (0))
			{
				...
				if (Time.realtimeSinceStartup - tapeTime[fingerIndex] > 0.5) tapCount[fingerIndex] = 0;
				...
				if (Input.GetMouseButtonDown (0) || (fingerIndex == 1 && (Input.GetKeyDown (KeyCode.LeftAlt) || Input.GetKeyDown (EasyTouch.instance.twistKey) || Input.GetKeyDown (KeyCode.LeftControl) || Input.GetKeyDown (EasyTouch.instance.swipeKey))))
				{
					...
					else oldMousePosition[fingerIndex] = finger.position;

					if (tapCount[fingerIndex] == 1) tapeTime[fingerIndex] = Time.realtimeSinceStartup;
				...
				finger.tapCount = tapCount[fingerIndex];
				finger.deltaTime = Time.realtimeSinceStartup - deltaTime[fingerIndex];
				if (finger.deltaPosition.sqrMagnitude < 1) finger.phase = TouchPhase.Stationary;
				else finger.phase = TouchPhase.Moved;
		...
		Vector2 GetPointerPosition (int index)
		{
			Vector2 pos;
			...
			else return GetSecondFingerPosition ();
		}

		// Simulate for a twist or pinc
		Vector2 GetPinchTwist2Finger (bool newSim = false)
		{
			Vector2 position;
