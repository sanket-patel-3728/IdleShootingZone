...
public class ETCJoystickInspector : Editor
{
	...
	public override void OnInspectorGUI ()
	{
		...
		if (t.showPSInspector)
		{
			ETCGuiTools.BeginGroup ();
			{
				...
				if (t.joystickType == ETCJoystick.JoystickType.Static)
				{
					...
					if (t.anchor != ETCBase.RectAnchor.UserDefined) t.anchorOffet = EditorGUILayout.Vector2Field ("Offset", t.anchorOffet);
				...
				else if (t.joystickType == ETCJoystick.JoystickType.Dynamic)
				{
					...
					if (t.joystickArea == ETCJoystick.JoystickArea.UserDefined) t.userArea = (RectTransform)EditorGUILayout.ObjectField ("User area", t.userArea, typeof (RectTransform), true);
				}
				...
				if (t.radiusBase == ETCJoystick.RadiusBase.UserDefined) t.radiusBaseValue = EditorGUILayout.FloatField ("Radius", t.radiusBaseValue);
				EditorGUILayout.Space ();
			}
		...
		if (t.showAxesInspector)
		{
			ETCGuiTools.BeginGroup ();
			{
				...
				ETCGuiTools.BeginGroup (5);
				{
					...
					if (t.isTurnAndMove) TurnAndMove (t.axisX, t.axisY, t);
				}
				ETCGuiTools.EndGroup ();
			...
			ETCGuiTools.BeginGroup ();
			{
				...
				if (t.enableCamera)
				{
					...
					ETCGuiTools.BeginGroup (5);
					{
						...
						if (t.autoLinkTagCam) t.autoCamTag = EditorGUILayout.TagField ("", t.autoCamTag);
						else t.cameraTransform = (Transform)EditorGUILayout.ObjectField ("Camera", t.cameraTransform, typeof (Transform), true);
					}
					ETCGuiTools.EndGroup ();
					...
					ETCGuiTools.BeginGroup (5);
					{
						t.cameraTargetMode = (ETCBase.CameraTargetMode)EditorGUILayout.EnumPopup ("Target mode", t.cameraTargetMode);
						if (t.cameraTargetMode == ETCBase.CameraTargetMode.UserDefined) t.cameraLookAt = (Transform)EditorGUILayout.ObjectField ("Camera target", t.cameraLookAt, typeof (Transform), true);
						if (t.cameraTargetMode == ETCBase.CameraTargetMode.LinkOnTag) t.camTargetTag = EditorGUILayout.TagField ("", t.camTargetTag);
					}
					ETCGuiTools.EndGroup ();
					...
					ETCGuiTools.BeginGroup (5);
					{
						t.cameraMode = (ETCBase.CameraMode)EditorGUILayout.EnumPopup ("Camera mode", t.cameraMode);
						switch (t.cameraMode)
						{
							case ETCBase.CameraMode.Follow:
							t.followOffset = EditorGUILayout.Vector3Field ("Offset", t.followOffset);
							break;
							case ETCBase.CameraMode.SmoothFollow:
		...
		if (t.anchor != ETCBase.RectAnchor.UserDefined && t.joystickType == ETCJoystick.JoystickType.Static) t.SetAnchorPosition ();
	...
	void TurnAndMove (ETCAxis X, ETCAxis Y, ETCJoystick j)
	{
		EditorGUI.indentLevel++;
		X.autoLinkTagPlayer = EditorGUILayout.ToggleLeft ("Auto link on tag", X.autoLinkTagPlayer, GUILayout.Width (200));
		if (X.autoLinkTagPlayer) X.autoTag = EditorGUILayout.TagField ("", X.autoTag);

		if (!X.autoLinkTagPlayer) X.directTransform = (Transform)EditorGUILayout.ObjectField ("Direct action to", X.directTransform, typeof (Transform), true);
		EditorGUILayout.Space ();

		if (j.tmMoveCurve == null) j.InitTurnMoveCurve ();
