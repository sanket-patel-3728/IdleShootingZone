...
public class ETCGuiTools
{
	...
	public static bool BeginFoldOut (string text, bool foldOut, bool endSpace = true)
	{
		...
		if (!GUILayout.Toggle (true, text, "dragtab")) foldOut = !foldOut;
	...
	public static void EndGroup (bool endSpace = true)
	{
		...
		if (endSpace) GUILayout.Space (10f);
	}
	...
	public static bool Toggle (string text, bool value, bool leftToggle = false, int width = -1, bool bold = false)
	{
		...
		if (leftToggle)
		{
			if (width == -1) value = EditorGUILayout.ToggleLeft (text, value);
			else value = GUILayout.Toggle (value, text, boldStyle, GUILayout.Width (width));
		}
		else
		{
			if (width == -1) value = EditorGUILayout.Toggle (text, value);
			else value = GUILayout.Toggle (value, text, boldStyle, GUILayout.Width (width));
		}
		return value;
	}
	...
	static public bool Button (string label, Color color, int width, bool leftAligment = false, int height = 0)
	{
		...
		if (leftAligment) buttonStyle.alignment = TextAnchor.MiddleLeft;
	...
	static void DrawLine (Color color, int padding = 0)
	{
		GUILayout.Space (10);
	...
	public static void DrawTextureRectPreview (Rect rect, Rect textureRect, Texture2D tex, Color color)
	{
		...
		DrawTileTexture (rect, GetCheckerTexture ());

		if (tex != null) GUI.DrawTextureWithTexCoords (rect, tex, textureRect);
	}
	...
	public static void DrawTileTexture (Rect rect, Texture tex)
	{
		GUI.BeginGroup (rect);
		{
			...
			{
				for (int x = 0; x < width; x += tex.width) GUI.DrawTexture (new Rect (x, y, tex.width, tex.height), tex);
			}
		}
		GUI.EndGroup ();
	}

	static Rect DrawGradient (int padding, int width, int height = 35)
	{
	...
	{
		if (gradientTexture == null) gradientTexture = CreateGradientTexture ();
	...
	{
		if (checkerTexture == null) checkerTexture = CreateCheckerTexture ();
