...
public class ETCButtonInspector : Editor
{
	...
	public override void OnInspectorGUI ()
	{
			...
			ETCGuiTools.BeginGroup ();
			{
				...
				if (t.axis.showDirectInspector)
				{
					ETCGuiTools.BeginGroup (20);
					{
						...
						if (t.axis.autoLinkTagPlayer) t.axis.autoTag = EditorGUILayout.TagField ("", t.axis.autoTag);
						else t.axis.directTransform = (Transform)EditorGUILayout.ObjectField ("Direct action to", t.axis.directTransform, typeof (Transform), true);
						...
						if (t.axis.directAction != ETCAxis.DirectAction.Jump) t.axis.axisInfluenced = (ETCAxis.AxisInfluenced)EditorGUILayout.EnumPopup ("Affected axis", t.axis.axisInfluenced);
						else
						{
				...
				if (t.axis.showSimulatinInspector)
				{
					ETCGuiTools.BeginGroup (20);
					{
						...
						if (tmpIndex != index) t.axis.unityAxis = unityAxes[tmpIndex];
						EditorGUI.indentLevel++;
					}
					ETCGuiTools.EndGroup ();
				}
		...
		if (t.anchor != ETCBase.RectAnchor.UserDefined) t.SetAnchorPosition ();
