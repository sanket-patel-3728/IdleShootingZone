...
public class ETCAxis
{
	...
	[SerializeField] Transform _directTransform;
	...
	public void UpdateAxis (float realValue, bool isOnDrag, ETCBase.ControlType type, bool deltaTime = true)
	{
		...
		{
			player = GameObject.FindGameObjectWithTag (autoTag);
			if (player) directTransform = player.transform;
		}

		// Auto stabilization
		if (isAutoStab && axisValue == 0 && _directTransform) DoAutoStabilisation ();
	...
	public void UpdateButton ()
	{
		...
		{
			if (axisState == AxisState.Press || axisState == AxisState.Down) axisValue = 1;
			else axisValue = 0;
		}
		...
		switch (actionOn)
		{
			...
			if (axisState == AxisState.Down) DoDirectAction ();
			break;
			case ActionOn.Press:
			axisSpeedValue = axisValue * speed * Time.deltaTime;
			if (axisState == AxisState.Press) DoDirectAction ();
			break;
		}
	}
	...
	private void ComputAxisValue (float realValue, ETCBase.ControlType type, bool isOnDrag, bool deltaTime)
	{
		if (enable)
		{
			...
			if (isEnertia)
			{
				...
				if (Mathf.Abs (axisValue) < inertiaThreshold && !isOnDrag) axisValue = 0;
			}
			else if (!isValueOverTime || (isValueOverTime && realValue == 0)) axisValue = realValue;

			if (deltaTime) axisSpeedValue = axisValue * speed * Time.deltaTime;
			else axisSpeedValue = axisValue * speed;
		}
		else
		{
			axisValue = 0;
	...
	float GetAngle ()
	{
		...
		if (_directTransform != null)
		{
			...
			if (angle <= 360 && angle >= 180) angle = angle - 360;
		}
	...
	void DoAutoStabilisation ()
	{
		...
		if (angle > startAngle - autoStabThreshold || angle < startAngle + autoStabThreshold)
		{
			...
			if (angle > startAngle - autoStabThreshold) axis = angle + autoStabSpeed / 100f * Mathf.Abs (angle - startAngle) * Time.deltaTime * -1;


			if (angle < startAngle + autoStabThreshold) axis = angle + autoStabSpeed / 100f * Mathf.Abs (angle - startAngle) * Time.deltaTime;
