...
public class ETCInput : MonoBehaviour
{
	...
	Dictionary<string, ETCAxis> axes = new Dictionary<string, ETCAxis> ();
	Dictionary<string, ETCBase> controls = new Dictionary<string, ETCBase> ();

	static ETCBase control;
	static ETCAxis axis;

	public void RegisterControl (ETCBase ctrl)
	{
		if (controls.ContainsKey (ctrl.name)) Debug.LogWarning ("ETCInput control : " + ctrl.name + " already exists");
		else
		{
	...
	public static void Register (ETCBase ctrl) => Instance.RegisterControl (ctrl);

	public static void UnRegister (ETCBase ctrl) => Instance.UnRegisterControl (ctrl);


	public static void SetControlVisible (string ctrlName, bool value)
	{
		if (Instance.controls.TryGetValue (ctrlName, out control)) control.visible = value;
		else Debug.LogWarning ("ETCInput : " + ctrlName + " doesn't exist");
	}

	public static bool GetControlVisible (string ctrlName)
	{
		if (Instance.controls.TryGetValue (ctrlName, out control)) return control.visible;
		else
		{
	...
	{
		if (Instance.controls.TryGetValue (ctrlName, out control)) control.activated = value;
		else Debug.LogWarning ("ETCInput : " + ctrlName + " doesn't exist");
	}

	public static bool GetControlActivated (string ctrlName)
	{
		if (Instance.controls.TryGetValue (ctrlName, out control)) return control.activated;
		else
		{
			Debug.LogWarning ("ETCInput : " + ctrlName + " doesn't exist");
			return false;
	...
	{
		if (Instance.controls.TryGetValue (ctrlName, out control)) control.isSwipeIn = value;
		else Debug.LogWarning ("ETCInput : " + ctrlName + " doesn't exist");
	}

	public static bool GetControlSwipeIn (string ctrlName)
	{
		if (Instance.controls.TryGetValue (ctrlName, out control)) return control.isSwipeIn;
		else
		{
			Debug.LogWarning ("ETCInput : " + ctrlName + " doesn't exist");
			return false;
	...
	{
		if (Instance.controls.TryGetValue (ctrlName, out control)) control.isSwipeOut = value;
		else Debug.LogWarning ("ETCInput : " + ctrlName + " doesn't exist");
	}

	public static bool GetControlSwipeOut (string ctrlName, bool value)
	{
		if (Instance.controls.TryGetValue (ctrlName, out control)) return control.isSwipeOut;
		else
		{
			Debug.LogWarning ("ETCInput : " + ctrlName + " doesn't exist");
			return false;
	...
	{
		if (Instance.controls.TryGetValue (ctrlName, out control)) control.dPadAxisCount = value;
		else Debug.LogWarning ("ETCInput : " + ctrlName + " doesn't exist");
	}

	public static ETCBase.DPadAxis GetDPadAxesCount (string ctrlName)
	{
		if (Instance.controls.TryGetValue (ctrlName, out control)) return control.dPadAxisCount;
		else
		{
			Debug.LogWarning ("ETCInput : " + ctrlName + " doesn't exist");
	...
	{
		if (Instance.axes.TryGetValue (axisName, out axis)) axis.speed = speed;
		else Debug.LogWarning ("ETCInput : " + axisName + " doesn't exist");
	}

	public static void SetAxisGravity (string axisName, float gravity)
	{
		if (Instance.axes.TryGetValue (axisName, out axis)) axis.gravity = gravity;
		else Debug.LogWarning ("ETCInput : " + axisName + " doesn't exist");
	}
	...
	{
		if (Instance.axes.TryGetValue (axisName, out axis)) axis.enable = value;
		else Debug.LogWarning ("ETCInput : " + axisName + " doesn't exist");
	}

	public static bool GetAxisEnabled (string axisName)
	{
		if (Instance.axes.TryGetValue (axisName, out axis)) return axis.enable;
		else
		{
	...
	{
		if (Instance.axes.TryGetValue (axisName, out axis)) axis.invertedAxis = value;
		else Debug.LogWarning ("ETCInput : " + axisName + " doesn't exist");
	}

	public static bool GetAxisInverted (string axisName)
	{
		if (Instance.axes.TryGetValue (axisName, out axis)) return axis.invertedAxis;
		else
		{
			Debug.LogWarning ("ETCInput : " + axisName + " doesn't exist");
			return false;
	...
	{
		if (Instance.axes.TryGetValue (axisName, out axis)) axis.deadValue = value;
		else Debug.LogWarning ("ETCInput : " + axisName + " doesn't exist");
	}

	public static float GetAxisDeadValue (string axisName)
	{
		if (Instance.axes.TryGetValue (axisName, out axis)) return axis.deadValue;
		else
		{
			Debug.LogWarning ("ETCInput : " + axisName + " doesn't exist");
			return -1;
	...
	{
		if (Instance.axes.TryGetValue (axisName, out axis)) axis.speed = value;
		else Debug.LogWarning ("ETCInput : " + axisName + " doesn't exist");
	}

	public static float GetAxisSensitivity (string axisName)
	{
		if (Instance.axes.TryGetValue (axisName, out axis)) return axis.speed;
		else
		{
			Debug.LogWarning ("ETCInput : " + axisName + " doesn't exist");
			return -1;
	...
	{
		if (Instance.axes.TryGetValue (axisName, out axis)) axis.axisThreshold = value;
		else Debug.LogWarning ("ETCInput : " + axisName + " doesn't exist");
	}

	public static float GetAxisThreshold (string axisName)
	{
		if (Instance.axes.TryGetValue (axisName, out axis)) return axis.axisThreshold;
		else
		{
			Debug.LogWarning ("ETCInput : " + axisName + " doesn't exist");
			return -1;
	...
	{
		if (Instance.axes.TryGetValue (axisName, out axis)) axis.isEnertia = value;
		else Debug.LogWarning ("ETCInput : " + axisName + " doesn't exist");
	}

	public static bool GetAxisInertia (string axisName)
	{
		if (Instance.axes.TryGetValue (axisName, out axis)) return axis.isEnertia;
		else
		{
			Debug.LogWarning ("ETCInput : " + axisName + " doesn't exist");
			return false;
	...
	{
		if (Instance.axes.TryGetValue (axisName, out axis)) axis.inertia = value;
		else Debug.LogWarning ("ETCInput : " + axisName + " doesn't exist");
	}

	public static float GetAxisInertiaSpeed (string axisName)
	{
		if (Instance.axes.TryGetValue (axisName, out axis)) return axis.inertia;
		else
		{
			Debug.LogWarning ("ETCInput : " + axisName + " doesn't exist");
			return -1;
	...
	{
		if (Instance.axes.TryGetValue (axisName, out axis)) axis.inertiaThreshold = value;
		else Debug.LogWarning ("ETCInput : " + axisName + " doesn't exist");
	}

	public static float GetAxisInertiaThreshold (string axisName)
	{
		if (Instance.axes.TryGetValue (axisName, out axis)) return axis.inertiaThreshold;
		else
		{
			Debug.LogWarning ("ETCInput : " + axisName + " doesn't exist");
			return -1;
	...
	{
		if (Instance.axes.TryGetValue (axisName, out axis)) axis.isAutoStab = value;
		else Debug.LogWarning ("ETCInput : " + axisName + " doesn't exist");
	}

	public static bool GetAxisAutoStabilization (string axisName)
	{
		if (Instance.axes.TryGetValue (axisName, out axis)) return axis.isAutoStab;
		else
		{
			Debug.LogWarning ("ETCInput : " + axisName + " doesn't exist");
			return false;
	...
	{
		if (Instance.axes.TryGetValue (axisName, out axis)) axis.autoStabSpeed = value;
		else Debug.LogWarning ("ETCInput : " + axisName + " doesn't exist");
	}

	public static float GetAxisAutoStabilizationSpeed (string axisName)
	{
		if (Instance.axes.TryGetValue (axisName, out axis)) return axis.autoStabSpeed;
		else
		{
			Debug.LogWarning ("ETCInput : " + axisName + " doesn't exist");
			return -1;
	...
	{
		if (Instance.axes.TryGetValue (axisName, out axis)) axis.autoStabThreshold = value;
		else Debug.LogWarning ("ETCInput : " + axisName + " doesn't exist");
	}

	public static float GetAxisAutoStabilizationThreshold (string axisName)
	{
		if (Instance.axes.TryGetValue (axisName, out axis)) return axis.autoStabThreshold;
		else
		{
			Debug.LogWarning ("ETCInput : " + axisName + " doesn't exist");
			return -1;
	...
	{
		if (Instance.axes.TryGetValue (axisName, out axis)) axis.isClampRotation = value;
		else Debug.LogWarning ("ETCInput : " + axisName + " doesn't exist");
	}

	public static bool GetAxisClampRotation (string axisName)
	{
		if (Instance.axes.TryGetValue (axisName, out axis)) return axis.isClampRotation;
		else
		{
			Debug.LogWarning ("ETCInput : " + axisName + " doesn't exist");
			return false;
	...
	{
		if (Instance.axes.TryGetValue (axisName, out axis)) axis.minAngle = value;
		else Debug.LogWarning ("ETCInput : " + axisName + " doesn't exist");
	}

	public static void SetAxisClampRotationMaxValue (string axisName, float value)
	{
		if (Instance.axes.TryGetValue (axisName, out axis)) axis.maxAngle = value;
		else Debug.LogWarning ("ETCInput : " + axisName + " doesn't exist");
	}

	public static float GetAxisClampRotationMinValue (string axisName)
	{
		if (Instance.axes.TryGetValue (axisName, out axis)) return axis.minAngle;
		else
		{
			Debug.LogWarning ("ETCInput : " + axisName + " doesn't exist");
			return -1;
	...
	{
		if (Instance.axes.TryGetValue (axisName, out axis)) return axis.maxAngle;
		else
		{
			Debug.LogWarning ("ETCInput : " + axisName + " doesn't exist");
			return -1;
	...
	{
		if (Instance.axes.TryGetValue (axisName, out axis)) axis.directTransform = value;
		else Debug.LogWarning ("ETCInput : " + axisName + " doesn't exist");
	}

	public static Transform GetAxisDirectTransform (string axisName)
	{
		if (Instance.axes.TryGetValue (axisName, out axis)) return axis.directTransform;
		else
		{
			Debug.LogWarning ("ETCInput : " + axisName + " doesn't exist");
			return null;
	...
	{
		if (Instance.axes.TryGetValue (axisName, out axis)) axis.directAction = value;
		else Debug.LogWarning ("ETCInput : " + axisName + " doesn't exist");
	}

	public static ETCAxis.DirectAction GetAxisDirectAction (string axisName)
	{
		if (Instance.axes.TryGetValue (axisName, out axis)) return axis.directAction;
		else
		{
			Debug.LogWarning ("ETCInput : " + axisName + " doesn't exist");
	...
	{
		if (Instance.axes.TryGetValue (axisName, out axis)) axis.axisInfluenced = value;
		else Debug.LogWarning ("ETCInput : " + axisName + " doesn't exist");
	}

	public static ETCAxis.AxisInfluenced GetAxisAffectedAxis (string axisName)
	{
		if (Instance.axes.TryGetValue (axisName, out axis)) return axis.axisInfluenced;
		else
		{
			Debug.LogWarning ("ETCInput : " + axisName + " doesn't exist");
	...
	{
		if (Instance.axes.TryGetValue (axisName, out axis)) axis.isValueOverTime = value;
		else Debug.LogWarning ("ETCInput : " + axisName + " doesn't exist");
	}

	public static bool GetAxisOverTime (string axisName)
	{
		if (Instance.axes.TryGetValue (axisName, out axis)) return axis.isValueOverTime;
		else
		{
			Debug.LogWarning ("ETCInput : " + axisName + " doesn't exist");
			return false;
	...
	{
		if (Instance.axes.TryGetValue (axisName, out axis)) axis.overTimeStep = value;
		else Debug.LogWarning ("ETCInput : " + axisName + " doesn't exist");
	}

	public static float GetAxisOverTimeStep (string axisName)
	{
		if (Instance.axes.TryGetValue (axisName, out axis)) return axis.overTimeStep;
		else
		{
			Debug.LogWarning ("ETCInput : " + axisName + " doesn't exist");
			return -1;
	...
	{
		if (Instance.axes.TryGetValue (axisName, out axis)) axis.maxOverTimeValue = value;
		else Debug.LogWarning ("ETCInput : " + axisName + " doesn't exist");
	}

	public static float GetAxisOverTimeMaxValue (string axisName)
	{
		if (Instance.axes.TryGetValue (axisName, out axis)) return axis.maxOverTimeValue;
		else
		{
			Debug.LogWarning ("ETCInput : " + axisName + " doesn't exist");
			return -1;
	...
	{
		if (Instance.axes.TryGetValue (axisName, out axis)) return axis.axisValue;
		else
		{
			Debug.LogWarning ("ETCInput : " + axisName + " doesn't exist");
			return 0;
	...
	{
		if (Instance.axes.TryGetValue (axisName, out axis)) return axis.axisSpeedValue;
		else
		{
	...
	public static bool GetAxisDownUp (string axisName)
	{
		if (Instance.axes.TryGetValue (axisName, out axis))
		{
			if (axis.axisState == ETCAxis.AxisState.DownUp) return true;
			else return false;
		}
		else
		{
	...
	public static bool GetAxisDownDown (string axisName)
	{
		if (Instance.axes.TryGetValue (axisName, out axis))
		{
			if (axis.axisState == ETCAxis.AxisState.DownDown) return true;
			else return false;
		}
		else
		{
	...
	public static bool GetAxisDownRight (string axisName)
	{
		if (Instance.axes.TryGetValue (axisName, out axis))
		{
			if (axis.axisState == ETCAxis.AxisState.DownRight) return true;
			else return false;
		}
		else
		{
	...
	public static bool GetAxisDownLeft (string axisName)
	{
		if (Instance.axes.TryGetValue (axisName, out axis))
		{
			if (axis.axisState == ETCAxis.AxisState.DownLeft) return true;
			else return false;
		}
		else
		{
	...
	public static bool GetAxisPressedUp (string axisName)
	{
		if (Instance.axes.TryGetValue (axisName, out axis))
		{
			if (axis.axisState == ETCAxis.AxisState.PressUp) return true;
			else return false;
		}
		else
		{
	...
	public static bool GetAxisPressedDown (string axisName)
	{
		if (Instance.axes.TryGetValue (axisName, out axis))
		{
			if (axis.axisState == ETCAxis.AxisState.PressDown) return true;
			else return false;
		}
		else
		{
	...
	public static bool GetAxisPressedRight (string axisName)
	{
		if (Instance.axes.TryGetValue (axisName, out axis))
		{
			if (axis.axisState == ETCAxis.AxisState.PressRight) return true;
			else return false;
		}
		else
		{
	...
	public static bool GetAxisPressedLeft (string axisName)
	{
		if (Instance.axes.TryGetValue (axisName, out axis))
		{
			if (axis.axisState == ETCAxis.AxisState.PressLeft) return true;
			else return false;
		}
		else
		{
	...
	public static bool GetButtonDown (string buttonName)
	{
		...
		{
			if (axis.axisState == ETCAxis.AxisState.Down) return true;
			else return false;
		}
		else
		{
	...
	public static bool GetButton (string buttonName)
	{
		if (Instance.axes.TryGetValue (buttonName, out axis))
		{
			if (axis.axisState == ETCAxis.AxisState.Down || axis.axisState == ETCAxis.AxisState.Press) return true;
			else return false;
		}
		else
		{
	...
	public static bool GetButtonUp (string buttonName)
	{
		if (Instance.axes.TryGetValue (buttonName, out axis))
		{
			if (axis.axisState == ETCAxis.AxisState.Up) return true;
			else return false;
		}
		else
		{
	...
	{
		if (Instance.axes.TryGetValue (buttonName, out axis)) return axis.axisValue;
		else
		{
			Debug.LogWarning (buttonName + " doesn't exist");
			return -1;
	...
	void RegisterAxis (ETCAxis axis)
	{
		if (Instance.axes.ContainsKey (axis.name)) Debug.LogWarning ("ETCInput axis : " + axis.name + " already exists");
		else axes.Add (axis.name, axis);
	}

	void UnRegisterAxis (ETCAxis axis)
	{
		if (Instance.axes.ContainsKey (axis.name)) axes.Remove (axis.name);
	}
}