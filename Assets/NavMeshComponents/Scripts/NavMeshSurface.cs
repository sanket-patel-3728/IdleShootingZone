...
namespace UnityEngine.AI
{
	...
	public class NavMeshSurface : MonoBehaviour
	{
		...
		public void BuildNavMesh ()
		{
			...
			if ( m_CollectObjects == CollectObjects.All || m_CollectObjects == CollectObjects.Children ) sourcesBounds = CalculateWorldBounds ( sources );
		...
		{
			if ( activeSurfaces.Count == 0 ) NavMesh.onPreUpdate += UpdateActive;
			if ( !activeSurfaces.Contains ( surface ) ) activeSurfaces.Add ( surface );
		}
		...
		{
			for ( var i = 0 ; i < activeSurfaces.Count ; ++i ) activeSurfaces [ i ].UpdateDataIfTransformChanged ();
		}
		...
		void AppendModifierVolumes ( ref List<NavMeshBuildSource> sources )
		{
			...
			else modifiers = NavMeshModifierVolume.activeModifiers;

			foreach ( var m in modifiers )
			{
				if ( ( m_LayerMask & ( 1 << m.gameObject.layer ) ) == 0 ) continue;
				if ( !m.AffectsAgentType ( m_AgentTypeID ) ) continue;
		...
		List<NavMeshBuildSource> CollectSources ()
		{
			...
			if ( m_CollectObjects == CollectObjects.All ) NavMeshBuilder.CollectSources ( null , m_LayerMask , m_UseGeometry , m_DefaultArea , markups , sources );
			else if ( m_CollectObjects == CollectObjects.Children ) NavMeshBuilder.CollectSources ( transform , m_LayerMask , m_UseGeometry , m_DefaultArea , markups , sources );
		...
		bool UnshareNavMeshAsset ()
		{
			...
			if ( prefabType == UnityEditor.PrefabAssetType.Regular ) return false;

			// An instance can share asset reference only with its prefab parent
			var prefab = UnityEditor.PrefabUtility.GetCorrespondingObjectFromSource ( this ) as NavMeshSurface;
			if ( prefab != null && prefab.navMeshData == navMeshData ) return false;
			...
			for ( var i = 0 ; i < activeSurfaces.Count ; ++i )
			{
				...
				if ( surface != this && surface.m_NavMeshData == m_NavMeshData ) return true;
			}
		...
		void OnValidate ()
		{
			...
			if ( settings.agentTypeID != -1 )
			{
				...
				if ( !m_OverrideVoxelSize ) m_VoxelSize = settings.agentRadius / 3.0f;
				if ( m_VoxelSize < kMinVoxelSize ) m_VoxelSize = kMinVoxelSize;
				...
				if ( !m_OverrideTileSize ) m_TileSize = kDefaultTileSize;
				// Make sure tilesize is in sane range.
				if ( m_TileSize < kMinTileSize ) m_TileSize = kMinTileSize;
				if ( m_TileSize > kMaxTileSize ) m_TileSize = kMaxTileSize;
			}
		}
#endif
	}
