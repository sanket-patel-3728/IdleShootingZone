...
namespace UnityEngine.AI
{
	...
	public class NavMeshLink : MonoBehaviour
	{
		...
		{
			AddLink ();
			if ( m_AutoUpdatePosition && m_LinkInstance.valid ) AddTracking ( this );
		}
...
#endif
			if ( s_Tracked.Count == 0 ) NavMesh.onPreUpdate += UpdateTrackedInstances;
			s_Tracked.Add ( link );
		}
		...
		{
			if ( m_AutoUpdatePosition == value ) return;
			m_AutoUpdatePosition = value;
			if ( value ) AddTracking ( this );
			else RemoveTracking ( this );
		}
		...
		void AddLink ()
		{
			...
			if ( m_LinkInstance.valid ) m_LinkInstance.owner = this;
		...
		static void UpdateTrackedInstances ()
		{
			...
			{
				if ( instance.HasTransformChanged () ) instance.UpdateLink ();
			}
		}
		...
		void OnValidate ()
		{
			...
			if ( !m_LinkInstance.valid ) return;

			UpdateLink ();

			if ( !m_AutoUpdatePosition ) RemoveTracking ( this );
