	...
	class NavMeshSurfaceEditor : Editor
	{
		...
		static NavMeshData GetNavMeshAssetToDelete ( NavMeshSurface navSurface )
		{
			...
			if ( prefabType == PrefabAssetType.Regular || prefabType == PrefabAssetType.MissingAsset )
			{
				...
				if ( parentSurface && navSurface.navMeshData == parentSurface.navMeshData ) return null;
			}
		...
		public override void OnInspectorGUI ()
		{
			...
			{
				if ( editingCollider ) EditMode.QuitEditMode ();
			}
			...
			foreach ( NavMeshSurface navSurface in targets )
			{
				...
				if ( navSurface.collectObjects == CollectObjects.Volume ) bounds = new Bounds ( navSurface.center , navSurface.size );
			...
			using ( new EditorGUI.DisabledScope ( Application.isPlaying || m_AgentTypeID.intValue == -1 ) )
			{
				...
				{
					foreach ( NavMeshSurface s in targets ) ClearSurface ( s );
		...
		static void UpdateAsyncBuildOperations ()
		{
			...
			{
				if ( oper.surface == null || oper.bakeOperation == null ) continue;

				if ( oper.bakeOperation.isDone )
				{
					var surface = oper.surface;
					var delete = GetNavMeshAssetToDelete ( surface );
					if ( delete != null ) AssetDatabase.DeleteAsset ( AssetDatabase.GetAssetPath ( delete ) );
			...
			if ( s_BakeOperations.Count == 0 ) EditorApplication.update -= UpdateAsyncBuildOperations;
		}

		[DrawGizmo ( GizmoType.Selected | GizmoType.Active | GizmoType.Pickable )]
		static void RenderBoxGizmoSelected ( NavMeshSurface navSurface , GizmoType gizmoType ) => RenderBoxGizmo ( navSurface , gizmoType , true );

		[DrawGizmo ( GizmoType.NotInSelectionHierarchy | GizmoType.Pickable )]
		static void RenderBoxGizmoNotSelected ( NavMeshSurface navSurface , GizmoType gizmoType )
		{
			if ( NavMeshVisualizationSettings.showNavigation > 0 ) RenderBoxGizmo ( navSurface , gizmoType , false );
			else Gizmos.DrawIcon ( navSurface.transform.position , "NavMeshSurface Icon" , true );
		}
		...
		static void RenderBoxGizmo ( NavMeshSurface navSurface , GizmoType gizmoType , bool selected )
		{
			...
			if ( !navSurface.enabled ) color = s_HandleColorDisabled;
